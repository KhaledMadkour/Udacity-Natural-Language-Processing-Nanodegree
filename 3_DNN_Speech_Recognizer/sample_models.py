...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for i in range(recur_layers):
        simp_rnn = GRU(units, activation='relu',
            return_sequences=True, implementation=2, name='bi_rnn_{}'.format(i))(bn_rnn)
...
def final_model(input_dim, filters, kernel_size, conv_stride,
    ...
    bn_cnn = BatchNormalization(name='bn_conv_1d')(conv_1d)
    bn_rnn = bn_cnn
    # Add a multi bidirectional recurrent layer
    for i in range(recur_layers):
        bi_rnn = Bidirectional(GRU(units, activation='relu',
            return_sequences=True, dropout = 0.3 , name='bi_rnn_{}'.format(i)), merge_mode='concat')(bn_rnn)
        bn_rnn = BatchNormalization(name='bn_rnn_{}'.format(i))(bi_rnn)
